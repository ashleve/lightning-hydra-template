...
python train.py trainer=debug_trainer debug=True \
print_config=false

echo "TEST 2"
echo "Overfit to 10 batches (10 epochs)"
python train.py trainer=debug_trainer trainer.overfit_batches=10 \
trainer.min_epochs=10 trainer.max_epochs=10 \
print_config=false

echo "TEST 3"
echo "Train on CPU (1 epoch)"
python train.py trainer=debug_trainer trainer.gpus=0 trainer.max_epochs=1 \
print_config=false

echo "TEST 4"
echo "Train on 25% of data (1 epoch)"
python train.py trainer=debug_trainer trainer.max_epochs=1 \
...
python train.py -m trainer=debug_trainer '+experiment=glob(*)' trainer.max_epochs=2 \
print_config=false

echo "TEST 7"
echo "Run default hydra sweep (executes 4 different combinations in debug mode)"
python train.py -m trainer=debug_trainer datamodule.batch_size=32,64 optimizer.lr=0.001,0.003 \
debug=True \
print_config=false

echo "TEST 8"
echo "Run with gradient accumulation (1 epoch)"
python train.py trainer=debug_trainer trainer.max_epochs=1 trainer.accumulate_grad_batches=10 \
print_config=false

echo "TEST 9"
echo "Run validation loop twice per epoch (2 epochs)"
python train.py trainer=debug_trainer trainer.max_epochs=2 trainer.val_check_interval=0.5 \
print_config=false

echo "TEST 10"
echo "Run with CSVLogger (2 epochs)"
python train.py trainer=debug_trainer logger=csv trainer.min_epochs=5 trainer.max_epochs=2 \
print_config=false

echo "TEST 11"
echo "Run with TensorBoardLogger (2 epochs)"
python train.py trainer=debug_trainer logger=tensorboard trainer.min_epochs=5 trainer.max_epochs=2 \
print_config=false
